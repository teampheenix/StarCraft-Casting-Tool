...
class SubwindowIcons(PyQt5.QtWidgets.QWidget):
    ...
    def createWindow(self, mainWindow, controller):
        ...
        self.team1_icon = DragImageLabel(self.controller.logoManager.getTeam1().getAbsFile())
        layout.addWidget(self.team1_icon,0,0,5,1)
        layout.addWidget(PyQt5.QtWidgets.QPushButton(_("Load from File")),0,1)
        layout.addWidget(PyQt5.QtWidgets.QPushButton(_("Load from URL")),1,1)
        layout.addWidget(PyQt5.QtWidgets.QPushButton(_("Search Liquipedia")),2,1)
        layout.addWidget(PyQt5.QtWidgets.QPushButton(_("Add to Favorites")),3,1)
        ...
        layout = PyQt5.QtWidgets.QGridLayout()
        self.team2_icon = DragImageLabel(self.controller.logoManager.getTeam2().getAbsFile())
        layout.addWidget(self.team2_icon,0,1,5,1)
        layout.addWidget(PyQt5.QtWidgets.QPushButton(_("Load from File")),0,0)
        layout.addWidget(PyQt5.QtWidgets.QPushButton(_("Load from URL")),1,0)
        layout.addWidget(PyQt5.QtWidgets.QPushButton(_("Search Liquipedia")),2,0)
        layout.addWidget(PyQt5.QtWidgets.QPushButton(_("Add to Favorites")),3,0)
        ...
        self.fav_list.customContextMenuRequested.connect(self.listItemRightClickedFav)
        ...
        for logo in self.controller.logoManager.getFavorites():
            ...
            item = PyQt5.QtWidgets.QListWidgetItem(PyQt5.QtGui.QIcon(map),logo.getDesc())
        ...
        self.lastused_list.setContextMenuPolicy(PyQt5.QtCore.Qt.CustomContextMenu)
        self.lastused_list.customContextMenuRequested.connect(self.listItemRightClickedLastUsed)
        ...
        self.resize(PyQt5.QtCore.QSize(self.sizeHint().width(), mainWindow.size().height()*0.95))
    ...
    def listItemRightClickedLastUsed(self, QPos): 
        self.listMenu= PyQt5.QtWidgets.QMenu()
        menu_item = self.listMenu.addAction(_("Set Team 1 Logo"))
        menu_item.triggered.connect(lambda: self.setTeam1Logo(self.lastused_list))
        menu_item = self.listMenu.addAction(_("Set Team 2 Logo"))
        menu_item.triggered.connect(lambda: self.setTeam2Logo(self.lastused_list))
        menu_item = self.listMenu.addAction(_("Add to Favorites"))
        menu_item.triggered.connect(self.addFavoriteLastUsed)
        
        parentPosition = self.lastused_list.mapToGlobal(PyQt5.QtCore.QPoint(0, 0))        
        self.listMenu.move(parentPosition + QPos)
        self.listMenu.show() 
    ...
    def addFavoriteLastUsed(self):
        ...
        map = item.icon().pixmap(self.iconsize)
        map = map.scaled(self.iconsize, self.iconsize, PyQt5.QtCore.Qt.KeepAspectRatio)
        item = PyQt5.QtWidgets.QListWidgetItem(PyQt5.QtGui.QIcon(map),item.text())
        self.fav_list.addItem(item)
