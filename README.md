...
Frequently the order of players given by the SC2-Client-API differs from the order in the Observer-UI resulting in a swapped match score when using _Set Ingame Score_. To correct this you can activate Optical Character Recognition (OCR) once you have to downloaded and installed [Tesseract-OCR](https://github.com/UB-Mannheim/tesseract/wiki#tesseract-at-ub-mannheim) - it's recommended to use a [older](https://digi.bib.uni-mannheim.de/tesseract/) v3 version of Tesseract-OCR. On a dual monitor setup OCR will currently only work if SC2 is displayed on the primary monitor.
