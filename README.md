...
#### Set Ingame Score
By default SCC-Tool presses `CTRL+Shift+S` to toggle the Score in the Observer-UI. For an Oberserver-UI like WCS that has no toggle-able score and doesn't have assigned this hot-key combination it will be interpreted as `CTRL+S` and typically disable your sound. To prevent this, you can disable SCC-Tool pressing `CTRL+Shift+S` by setting `strgshifts = False` in your `config.ini` file while SCC-Tool is not running.
